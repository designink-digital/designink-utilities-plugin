...
if ( ! class_exists( '\Designink\WordPress\v1_0_0\Plugin\Module', false ) ) {
	...
	abstract class Module extends Singleton {
		...
		final protected function register_submodules() {
			...
			if ( is_dir( $modules_directory ) ) {
				$files = \Designink\WordPress\v1_0_0\Utility::scandir( $modules_directory );
		...
		final protected function construct_submodules() {
			foreach ( $this->loaded_modules as $Module ) {
				if ( $Module instanceof \Designink\WordPress\v1_0_0\Plugin\Module ) {
		...
		final protected function import_module( string $module_name, string $module_path ) {
			...
			if ( is_file( $module_path ) ) {
				...
				if ( class_exists( $qualified_name ) ) {
					$is_module = is_subclass_of( $qualified_name, 'Designink\WordPress\v1_0_0\Plugin\Module' );

					if ( $is_module ) {
						$Module = $qualified_name::submodule_instance();
						$this->loaded_modules[ $qualified_name ] = $Module;
					} else {
						$message_format = "Successfully found class, '%s', but it does not appear to be a Module, make sure you are implementing Designink\WordPress\v1_0_0\Plugin\Module in '%s'.";
		...
		/**
		 * An alias for Designink\WordPress\v1_0_0\Autoloader::add_autoload_directory()
		 * 
