...
if ( ! class_exists( '\Designink\WordPress\v1_0_0\Action_Scheduler\Cron_Manager', false ) ) {
	...
	final class Cron_Manager {
		...
		const WP_CRON_SCHEDULE = 'ds_action_cron_schedule';

		/** @var int The time (in seconds) between checks for timers */
		const WP_CRON_SCHEDULE_INTERVAL = ( 20 );

		/** @var string The name of the hook to run when firing the cron */
		const WP_CRON_SCHEDULE_HOOK = 'ds_action_scheduler_update_hook';
		...
		final public static function _cron_schedules( array $schedules ) {
			$schedules[ self::WP_CRON_SCHEDULE ] = array(
				...
				'display' => __( sprintf( 'Designink Action Scheduler Interval (set to %s seconds)', self::WP_CRON_SCHEDULE_INTERVAL ) ),
			);
		 ...
		 */
		final public static function _ds_action_scheduler_update_hook() {
